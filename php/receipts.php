...
try {
    ...
    $stmt = $conn->prepare("
        SELECT 
            ...
            modex, 
            cheque_date 
        FROM public.booking_payments where id=217
    ");
    ...
    $vch->addChild('DATE', $row['cheque_date']);
    $vch->addChild('VCHSTATUSDATE', $row['cheque_date']);
    $vch->addChild('GUID', 'ef1532b1-c551-4b3f-ac45-04402e1668cc-' . $guId_receipt);
    $vch->addChild('NARRATION', "Booking Payment - ID: " . $row['id']);
    ...
    $allLedgerEntries1->addChild('LEDGERNAME', 'Yash Chavan');
    ...
    $allLedgerEntries1->addChild('AMOUNT', $row['amount']);
    ...
    $billAllocations1->addChild('AMOUNT', $row['amount']);
    ...
    $allLedgerEntries2->addChild('LEDGERNAME', $row['modex']);
    ...
    $allLedgerEntries2->addChild('AMOUNT', $row['amount']);
    ...
    $allLedgerEntries2->addChild('CHEQUENUMBER', '123456'); // Example cheque number
    $allLedgerEntries2->addChild('CHEQUEDATE', $row['cheque_date']); // Example cheque date
    $allLedgerEntries2->addChild('CHEQUEBANKNAME', 'XYZ Bank'); // Example bank name
    $allLedgerEntries2->addChild('ISINCLUDEDINBOOKS', 'Yes'); // Is included in books (payment processed)
    $allLedgerEntries2->addChild('PAYMENTTYPE', $row['modex']); // Payment type is Cheque
    ...
    $billAllocations2->addChild('AMOUNT', $row['amount']); // The amount deducted by cheque
